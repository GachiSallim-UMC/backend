...
export class PasswordController {
  ...
  @Post('change-with-confirmation')
  @HttpCode(HttpStatus.OK)
  @UseGuards(CognitoAccessTokenGuard)
  @ApiBearerAuth('BearerAuth')
  @ApiOperation({
    summary: '비밀번호 변경(새 비밀번호 확인)',
    description:
      '현재 Cognito access token과 현재 비밀번호로 새 비밀번호를 설정하며, 새 비밀번호 확인값의 일치 여부를 검증합니다.',
  })
  @ApiHeader({
    name: 'Authorization',
    description: 'Cognito access token (Bearer JWT)',
  ...
  @ApiBody({ type: ChangePasswordWithConfirmationDto })
  @ApiResponse({ status: 200, description: '비밀번호 변경 성공', type: ChangePasswordResponseDto })
  @ApiResponse({
    status: 400,
    description: '현재 비밀번호 오류, 새 비밀번호 불일치 또는 새 비밀번호 정책 위반',
  })
  @ApiResponse({ status: 401, description: '인증 토큰이 없거나 올바르지 않음' })
  @ApiResponse({ status: 429, description: '인증 서비스 요청 제한 초과' })
  @ApiResponse({ status: 502, description: '인증 서비스 처리 오류' })
  changePasswordWithConfirmation(
    @CurrentAuth() auth: AuthContext,
    @Body() dto: ChangePasswordWithConfirmationDto,
  ): Promise<ChangePasswordResponseDto> {
    return this.passwordService.changePasswordWithConfirmation(auth.accessToken, dto);
  }
}
