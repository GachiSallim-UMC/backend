...
describe('RulesController', () => {
  it('throws COMMON_UNAUTHORIZED when x-user-id is missing on getRule', async () => {
    const rulesService = { getRule: jest.fn() } as unknown as RulesService;
    const controller = new RulesController(rulesService);

    await expect(controller.getRule(undefined as unknown as string, '1')).rejects.toMatchObject({
      code: ErrorCode.COMMON_UNAUTHORIZED,
    });
  });

  it('throws COMMON_BAD_REQUEST when user id is malformed on getRule', async () => {
    const rulesService = { getRule: jest.fn() } as unknown as RulesService;
    const controller = new RulesController(rulesService);

    await expect(controller.getRule('abc', '1')).rejects.toMatchObject({
      code: ErrorCode.COMMON_BAD_REQUEST,
    });
  });

  it('throws COMMON_BAD_REQUEST when ruleId is malformed on getRule', async () => {
    const rulesService = { getRule: jest.fn() } as unknown as RulesService;
    const controller = new RulesController(rulesService);

    await expect(controller.getRule('1', '12-3')).rejects.toMatchObject({
      code: ErrorCode.COMMON_BAD_REQUEST,
    });
  });
  ...
  it('forwards COMMON_NOT_FOUND from service on getRule', async () => {
    const getRule = jest.fn().mockRejectedValue(new BusinessException(ErrorCode.COMMON_NOT_FOUND));
    const rulesService = { getRule } as unknown as RulesService;
    const controller = new RulesController(rulesService);
